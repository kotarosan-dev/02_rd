...
flowchart TB
  subgraph "日々の業務（ここだけ人が触る）"
    U[成約・案件データをCRMに登録・更新]
  end

  subgraph 自動で動く部分
    S[QuickMLが定期でCRMからデータを取得]
    P[前処理・モデル訓練を自動実行]
    E[予測用のエンドポイントが更新される]
  end

  subgraph アプリ側の自動化
    API[紹介メール送信・マッチング率表示など]
  end
...
```mermaid
flowchart LR
  A[① CRMとQuickMLを接続] --> B[② 同期の間隔を決める 例 毎日毎週]
  B --> C[③ 前処理とモデル訓練のパイプラインを設計]
  C --> D[④ 同期のたびに自動実行 データ更新で自動再訓練にチェック]
  D --> E[⑤ エンドポイントを公開]
  E --> F[以降は自動＋APIで運用]
```
